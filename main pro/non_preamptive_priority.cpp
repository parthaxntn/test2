...
int main()
{
    ...
    int arrivaltime[n] = {0, 2, 1, 4, 6,5,7};
    int bursttime[n] = {3, 5, 4, 2, 9,4,10};
    int priority[n] = {2,6,3,5,7,4,10};
    int ATt[n];
    ...
    for (i = 0; i < n; i++)
    {
        cout << "P" << i + 1 << "\t\t" << bursttime[i] << "\t\t" << priority[i] << "\t\t" << arrivaltime[i] << "\t\t" <<arrivaltime[i]+turnaroundTime[i]<<"\t\t"<< waitingTime[i] << "\t\t" << turnaroundTime[i] << endl;
    }
